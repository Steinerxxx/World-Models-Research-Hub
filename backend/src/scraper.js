...
export async function scrapeArxiv(fullBackfill = false) {
  ...
  try {
    ...
    for (let start = 0; start < MAX_RESULTS && shouldContinue; start += BATCH_SIZE) {
      ...
      if (papers.length > 0) {
          console.log(`🤖 Applying AI accuracy check to ${papers.length} potential papers (Parallelized)...`);
          
          // Use parallel processing with a concurrency limit
          ...
          for (let i = 0; i < papers.length; i += CONCURRENCY_LIMIT) {
              ...
              const batchResults = await Promise.all(batch.map(async (paper) => {
                  try {
                      ...
                      console.error(`Error processing paper "${paper.title}":`, err);
                      return paper; // Keep it on error to be safe
                  }
              }));
              
              processedPapers.push(...batchResults.filter(p => p !== null));
              console.log(`   Progress: ${Math.min(i + CONCURRENCY_LIMIT, papers.length)}/${papers.length} processed`);
          }
          
          // Replace papers with AI-verified ones
          papers.length = 0;
          papers.push(...processedPapers);
          console.log(`   ✅ AI Check complete. Kept ${papers.length} relevant papers.`);
      }
